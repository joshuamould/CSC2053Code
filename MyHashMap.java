...
public class MyHashMap<K, V> implements MapInterface<K, V>
{
	...
	// If location is null, add entry to map, increase size, check if need to
	// enlarge
    ...
    public V remove(K k)
    // If an entry in this map with a key k exists then the value associated
    // with that entry is returned and the entry is removed; otherwise null is
    // returned.
    {
	...
	{
	    MapEntry holder = map[location];
	    map[location] = null;
	    numPairs--;
	    this.put((K) holder.getKey(), (V) holder.getValue());
	    location = (location + 1) % currCap;
	}
